...
public:
	...
	Entity(Mesh& mesh) : pMesh{ &mesh }
	{
		boundingVolume = std::make_unique<AABB>(generateAABB(mesh));
	...
	//	{
	//		ResourceManager::GetShader("shader").Use().SetMatrix4("model", transform.getModelMatrix());
	//		pMesh->Draw(ourShader);
	//		display++;
	//	}
	//	total++;
